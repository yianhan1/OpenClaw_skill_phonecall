      ...
      if (energy > 500) {
        ...
        if (++silenceFrames > 30) {
          ...
          if (fullPcm.length > 3200) {
            ...
            aborted = false;
            if (fullPcm.length > 16000) { console.log("[ws] Filler played at", Date.now()); playRandomFiller(ws, streamSid); } // only for longer utterances
            try {
